local tarotInfo = {
	set = "purgatorio",
  	key = "purg_rubicon",
    pools = {['purgatorio_cons'] = true},
  	pos = { x = 2, y = 2 },
  	cost = 3, 
  	atlas = "IDK",
  	loc_txt = {
    	name = "Rubicon",
		text = {
			"ballz",
		},
  	}, 
  	config = { extra = {}},
  	
  	loc_vars = function(self, info_queue, card) 
	  	return { vars = { } }
	end,

  	can_use = function(self, card) 
		local cards = get_highlighted_cards({ G.jokers, G.hand }, card, 1, 1, function(card)
			return true
		end)
		return #cards == 1
	end,

	use = function(self, card, area, copier)
		local cards = get_highlighted_cards({ G.jokers, G.hand }, card, 1, 1)
		if #cards == 1 and #SMODS.find_card('j_dydb_theotime') > 0 then
			if cards[1].area == G.jokers then
				cards[1]:remove_from_deck(true)

				local used_consumable = copier or card
				local deletable_jokers = {}
				for k, v in pairs(G.jokers.cards) do
					deletable_jokers[#deletable_jokers + 1] = v
				end
				local chosen_joker = 'j_dydb_theotime'
				local _first_dissolve = nil
				G.E_MANAGER:add_event(Event({
					trigger = "before",
					delay = 0.75,
					func = function()
						for k, v in pairs(deletable_jokers) do
							if v == chosen_joker then
								v:start_dissolve(nil, _first_dissolve)
								_first_dissolve = true
							end
						end
						return true
					end,
				}))

				G.jokers:remove_from_highlighted(cards[1])

				cards[1]:add_to_deck(true)
				G.E_MANAGER:add_event(Event({
					trigger = "after",
					delay = 0.4,
					func = function()
						play_sound("timpani")
						local card = create_card("Joker", G.jokers, nil, nil, nil, nil, 'dydb_autistic_creature', "dydb_rubicon")
						card:add_to_deck()
						G.jokers:emplace(card)
						card:juice_up(0.3, 0.5)
						return true
					end,
				}))
				delay(0.6)
			end
		end
	end,
}

return tarotInfo