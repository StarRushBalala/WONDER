local tarotInfo = {
  ...
  key = "void",
  pos = { x = 1, y = 0 }, -- this is the position of what sprite the consumable will use, balatro uses a 0 index system so the first sprite is 0,0 and the second sprite is 1,0 and so on
  ...
  use = function(self, card, area, copier) -- this is the function that is used to make our consumable do its thing in this case we will be using it to change a number of selected cards into Diamonds
    ...
    for i=1, #G.hand.highlighted do -- this is the loop that will change the selected cards into Diamonds, you can change 'Diamonds' to any other suit such as 'Hearts', 'Clubs', or 'Spades'
      G.E_MANAGER:add_event(Event({trigger = 'after',delay = 0.1,func = function() Card.set_ability(G.hand.highlighted[i], "m_dydb_void" );return true end})) -- this is where we tell the consumable to apply our custom enhancement
    end
    for i=1, #G.hand.highlighted do
