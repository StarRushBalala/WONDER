local tarotInfo = {
  ...
  key = "totoya",
  pos = { x = 6, y = 0 }, -- this is the position of what sprite the consumable will use, balatro uses a 0 index system so the first sprite is 0,0 and the second sprite is 1,0 and so on
  ...
  loc_txt = {
    name = "totya",
    text = {
      "{C:dark_edition}totoya{}",
    }
  }, -- this is the key that determines what atlas the consumable will use, this is the same as the key in the atlas function in main.lua
  ...
  config = { 
    extra = {
      max_selected = 5,
      mod_conv = "dydb_totoya",
    }
  }, -- this config determines how many cards can be selected when using the consumable, you can change this number to any number you want
  loc_vars = function(self, info_queue, card) -- this is the function that determines what variables will be shown when hovering over the consumable
    ...
    info_queue[#info_queue + 1] = G.P_CENTERS[card.ability.mod_conv]
        return { vars = { card.ability.max_highlighted, localize { type = 'name_text', set = 'Seal', key = card.ability.mod_conv } } }
  end,
  use = function(self, card, area, copier) -- this is the function that is used to make our consumable do its thing in this case we will be using it to change a number of selected cards into Diamonds
    ...
    for i=1, #G.hand.highlighted do
      ...
      G.E_MANAGER:add_event(Event({
        trigger = 'after',
        delay = 0.15,
        func = function() G.hand.highlighted[i]:flip();
    ...
    for i=1, #G.hand.highlighted do -- this is the loop that will change the selected cards into Diamonds, you can change 'Diamonds' to any other suit such as 'Hearts', 'Clubs', or 'Spades'
      G.E_MANAGER:add_event(Event({
        trigger = 'after',
        delay = 0.1,
        func = function() Card.set_seal(G.hand.highlighted[i], "dydb_totoya" );
          return true 
        end
      })) -- this is where we tell the consumable to apply our custom enhancement
    end
    for i=1, #G.hand.highlighted do
    ...
    G.E_MANAGER:add_event(Event({
      trigger = 'after', 
      delay = 0.2,func = function() G.hand:unhighlight_all(); 
        return true 
      end 
    }))
    delay(0.5)
